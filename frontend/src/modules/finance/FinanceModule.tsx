...
export function FinanceModule({ finance, candidates, contracts, selectedContractId, onSelectContract, onPrepare, onUseCandidate, onUpload, onUploadFinance, onOpenSchedule, onReload, embedded = false }: Props) {
  ...
  if (embedded) return <section className="dds-intake" aria-label="Загрузка и разбор финансовых документов">
    <input ref={budgetInput} hidden aria-label="Файл бюджета" type="file" multiple accept=".xlsx,.xls,.csv,.pdf,.doc,.docx,.txt,.png,.jpg,.jpeg,.tif,.tiff,.bmp,.webp" onChange={(event) => upload("budget", event.target.files, event.currentTarget)} />
    <input ref={cashFlowInput} hidden aria-label="Файл планового ДДС" type="file" multiple accept=".xlsx,.xls,.csv,.pdf,.doc,.docx,.txt,.png,.jpg,.jpeg,.tif,.tiff,.bmp,.webp" onChange={(event) => upload("cash-flow", event.target.files, event.currentTarget)} />
    <div className="dds-intake-toolbar">
      <label><span>Финансовый договор</span><select aria-label="Финансовый договор" value={selectedContractId} onChange={(event) => onSelectContract(Number(event.target.value))}><option value={0}>Выберите наш финансовый договор</option>{financialContracts.map((item) => <option value={item.id} key={item.id}>{item.number} — {item.title}</option>)}</select></label>
      <div className="dds-intake-actions">
        <button type="button" disabled={!contract} onClick={() => budgetInput.current?.click()}>Загрузить бюджет</button>
        <button type="button" disabled={!contract} onClick={() => cashFlowInput.current?.click()}>Загрузить плановый ДДС</button>
        <button type="button" onClick={onUpload}>Загрузить счёт или акт</button>
        <button type="button" className="secondary" onClick={onReload}>Обновить анализ</button>
      </div>
    </div>
    ...
    <details className="dds-intake-candidates" open={visibleCandidates.length > 0}>
      <summary>Импорт и разбор документов <span>{visibleCandidates.length ? `${visibleCandidates.length} к проверке` : "нет новых предложений"}</span></summary>
