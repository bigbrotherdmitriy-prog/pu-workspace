...
export function FinanceModule({ finance, candidates, contracts, selectedContractId, onSelectContract, onPrepare, onUseCandidate, onReload }: Props) {
  ...
  return <>
    <p className="finance-warning">Показатели ниже относятся ко всему проекту, без фильтра выбранного договора. Периодный ДДС имеет отдельные фильтры; прогноз не равен подтверждённому факту.</p>
    ...
    <section className="card finance-contract-chain"><div><span className="eyebrow">ФИНАНСОВЫЙ КОНТУР НАШЕЙ КОМПАНИИ</span><h2>Договор → ГПР → бюджет → ДДС → акты</h2><p>Генподрядный договор остаётся контекстом объекта. Деньги и план работ учитываются только по нашему доходному или расходному договору.</p></div><select value={selectedContractId} onChange={(event) => onSelectContract(Number(event.target.value))}><option value={0}>Выберите наш финансовый договор</option>{financialContracts.map((item) => <option value={item.id} key={item.id}>{item.number} — {item.title}</option>)}</select></section>
