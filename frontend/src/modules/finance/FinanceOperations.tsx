...
export function FinanceOperations(props: Props) {
  ...
  return <>
    ...
    {includeEditor && preview && <section className="card structured-import structured-import-review" id="structured-import">
      ...
      <div className="structured-actions structured-review-actions"><span>Выбрано: <strong>{selectedRows.length}</strong></span><div><button className="secondary" onClick={onClosePreview}>Отмена</button><button disabled={!selectedRows.length} onClick={onImport}>Создать пакет предложений ({selectedRows.length})</button></div></div>
    </section>}
