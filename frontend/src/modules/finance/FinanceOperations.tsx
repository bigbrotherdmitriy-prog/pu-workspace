...
export function FinanceOperations(props: Props) {
  ...
  return <>
    ...
    {includeEditor && preview && <section className="card structured-import" id="structured-import">
      ...
      <div className="structured-table"><table><thead><tr><th></th><th>Источник</th><th>Объект</th><th>Наименование / статья</th><th>Тип</th><th>Дата / срок</th><th>Сумма / прогресс</th><th>Проверка</th></tr></thead><tbody>{preview.rows.slice(0, 100).map((row) => <tr className={row.importable ? "" : "invalid"} key={`${row.source_sheet || "text"}:${row.source_row}`}><td><input type="checkbox" disabled={!row.importable} checked={selectedRows.includes(row.source_row)} onChange={(event) => setSelectedRows((current) => event.target.checked ? [...current, row.source_row] : current.filter((value) => value !== row.source_row))} /></td><td title={row.source_name}>{row.source_coordinate}</td><td>{row.object_name || "—"}</td><td>{row.title || "—"}<small>{row.category}</small></td><td>{row.direction === "inflow" ? "Приход" : row.direction === "outflow" ? "Расход" : "—"}</td><td>{row.planned_date || row.planned_finish || row.planned_start || "—"}</td><td>{row.amount ? money(Number(row.amount)) : `${row.progress || 0}%`}</td><td>{row.issues.length ? row.issues.join("; ") : "готово к предложению"}</td></tr>)}</tbody></table></div>
    ...
    {includeEditor && <section className="card finance-entry" id="finance-entry">
      ...
      <div><select aria-label="Тип финансовой записи" value={kind} onChange={(event) => setKind(event.target.value)}><option value="budget">Строка бюджета</option><option value="cash-in">Поступление ДДС</option><option value="cash-out">Выплата ДДС</option><option value="invoice">Счёт → предложение ДДС</option><option value="procurement">Закупка / поставка</option><option value="act">Акт</option><option value="baseline">Версия ГПР</option><option value="schedule">Этап ГПР</option></select>
