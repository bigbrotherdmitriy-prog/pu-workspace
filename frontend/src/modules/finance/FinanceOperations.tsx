...
type Props = {
  ...
  onConfirmPayment: (id: number, amount: number, recordVersion: number) => void;
  onCloneBaseline: (id: number, version: number) => void;
  onOpenGraph?: (id: number) => void;
  onUpdateSchedule: (id: number, currentProgress: number) => void;
  onCorrectPayment: (id: number, amount: number, date: string, recordVersion: number) => void;
...
export function FinanceOperations(props: Props) {
  const { finance, preview, selectedRows, setSelectedRows, selectedContractId, kind, title, amount, date, extra,
    ...
    setScheduleItemId, setBudgetLineId, onClosePreview, onImport, onAdd, onConfirm, onCloneBaseline,
    onUpdateSchedule, onConfirmPayment, onCorrectPayment,
  ...
  const visibleBaselines = filterContract(finance?.baselines);
  ...
  return <>
    ...
    {includeEditor && <section className="card finance-entry" id="finance-entry">
      ...
      <div><select value={kind} onChange={(event) => setKind(event.target.value)}><option value="budget">Строка бюджета</option><option value="cash-in">Поступление ДДС</option><option value="cash-out">Выплата ДДС</option><option value="invoice">Счёт → предложение ДДС</option><option value="procurement">Закупка / поставка</option><option value="act">Акт</option><option value="baseline">Версия ГПР</option><option value="schedule">Этап ГПР</option></select>
        ...
        {["budget", "cash-in", "cash-out", "invoice"].includes(kind) && <select value={scheduleItemId} onChange={(event) => setScheduleItemId(Number(event.target.value))}><option value={0}>Связать с этапом ГПР (не выбран)</option>{finance?.schedule.filter((stage) => { const baseline = finance.baselines.find((row) => row.id === stage.baseline_id); return !selectedContractId || baseline?.contract_id === selectedContractId; }).map((stage) => <option key={stage.id} value={stage.id}>{stage.title}</option>)}</select>}
        {["cash-in", "cash-out", "invoice"].includes(kind) && <select value={budgetLineId} onChange={(event) => setBudgetLineId(Number(event.target.value))}><option value={0}>Связать со строкой бюджета (не выбрана)</option>{filterContract(finance?.budget).map((row) => <option key={row.id} value={row.id}>{row.description}</option>)}</select>}
        <button disabled={!title.trim()} onClick={onAdd}>Создать предложение</button>
      </div>
    </section>}
    {includeRegisters && <section className="finance-grid">
      <article className="card finance-schedule-register"><h2>ГПР: версии, план и факт</h2><p className="finance-source-note">План утверждённой версии неизменяем. Изменения вносите через новый черновик; факт записывается отдельно.</p><div className="finance-list">{visibleBaselines.map((item) => <div key={item.id}><span><strong>{item.name}</strong><small>Версия {item.version} · {item.is_current ? "текущий утверждённый план" : item.status === "draft" ? "черновик новой редакции" : "историческая версия"}</small></span><b>{item.status}</b>{item.status === "draft" && <button onClick={() => props.onOpenGraph ? props.onOpenGraph(item.id) : onConfirm("baselines", item.id, "approved")}>Проверить и утвердить</button>}{props.onOpenGraph && item.status !== "draft" && <button className="secondary" onClick={() => props.onOpenGraph?.(item.id)}>Открыть график</button>}{item.is_current && !visibleBaselines.some((row) => row.status === "draft") && <button className="secondary" onClick={() => onCloneBaseline(item.id, item.version)}>Новая версия</button>}</div>)}{!visibleBaselines.length && <p className="finance-empty">Добавьте первую версию ГПР.</p>}</div><div className="finance-plan-fact"><strong>Этапы: план / факт</strong>{visibleSchedule.map((stage) => { const baseline = visibleBaselines.find((row) => row.id === stage.baseline_id); return <div key={stage.id}><span><b>{stage.title}</b><small>План: {stage.planned_start || "—"} → {stage.planned_finish || "—"} · {stage.planned_progress}%</small><small>Факт: {stage.actual_start || "—"} → {stage.actual_finish || "—"} · {stage.actual_progress}%</small></span>{baseline?.is_current && <button className="secondary" onClick={() => onUpdateSchedule(stage.id, stage.actual_progress)}>Обновить факт</button>}</div>; })}{!visibleSchedule.length && <p className="finance-empty">Этапов ГПР пока нет.</p>}</div><p className="finance-warning">Отстающих работ: {finance?.summary.delayed_schedule || 0}</p></article>
      <article className="card"><h2>Бюджет</h2><div className="finance-list">{filterContract(finance?.budget).map((item) => <div key={item.id}><span><strong>{item.description}</strong><small>{item.category} · план {money(item.planned_amount)} · законтрактовано {money(item.committed_amount)} · факт {money(item.actual_amount)} · прогноз {money(item.forecast_amount)}</small><small>{item.review_status === "required" ? "Требуется ручная проверка источника" : `Контроль: ${item.review_status}`}{item.source_document_version_id ? ` · документ v#${item.source_document_version_id}` : " · первичный документ не связан"}</small></span><b>{item.status}</b>{item.status === "proposed" && <button onClick={() => onConfirm("budget", item.id, "approved")}>Подтвердить</button>}</div>)}{!filterContract(finance?.budget).length && <p className="finance-empty">Строк бюджета пока нет.</p>}</div></article>
      <article className="card"><h2>ДДС</h2><div className="finance-list">{filterContract(finance?.cash_flow).map((item) => <div key={item.id}><span><strong>{item.title}</strong><small>{item.direction === "inflow" ? "Поступление заказчика" : "Выплата"} · {item.planned_date} · {money(item.planned_amount)}{item.actual_date ? ` · факт ${item.actual_date}, ${money(item.actual_amount)}` : ""}</small><small>{item.review_status === "required" ? "Требуется ручная проверка источника" : `Контроль: ${item.review_status}`}{item.source_document_version_id ? ` · документ v#${item.source_document_version_id}` : " · первичный документ не связан"}</small></span><b>{item.status}</b>{item.status === "proposed" && <button onClick={() => onConfirm("cash-flow", item.id, "approved")}>Подтвердить план</button>}{item.status === "approved" && <button onClick={() => onConfirmPayment(item.id, Number(item.planned_amount), item.record_version)}>Подтвердить оплату — факт пользователем</button>}{["paid", "received"].includes(item.status) && item.actual_date && <button className="secondary" onClick={() => onCorrectPayment(item.id, Number(item.actual_amount), item.actual_date || "", item.record_version)}>Исправить факт оплаты</button>}</div>)}{!filterContract(finance?.cash_flow).length && <p className="finance-empty">План ДДС пока пуст.</p>}</div></article>
