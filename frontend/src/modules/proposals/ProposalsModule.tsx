...
type Props = { collapsed: boolean; proposals: Proposal[]; busyProposal: number; targetFolders: string[]; onOpenDocuments: () => void; onApproveSafe: (proposal: Proposal) => void; onConfirmSelected: (proposal: Proposal) => void; onApply: (proposal: Proposal) => void; onStandardize: (proposal: Proposal) => void; onRollback: (proposal: Proposal) => void; onDecision: (action: ProposalAction, decision: string) => void; onSave: (proposalId: number, actionId: number) => void; onEdit: (proposalId: number, actionId: number, patch: Partial<ProposalAction>) => void; onApplySource: (proposal: Proposal, action: ProposalAction) => void; onApplySourceBulk: (proposal: Proposal) => void };
  ...
  return <section className={`module-overlay ${props.collapsed ? "collapsed" : ""}`}><div className="module-page">
    ...
    <div className="proposal-list">
      {props.proposals.map((proposal) => {
        const selectedCount = proposal.actions.filter((action) => ["approved", "edited"].includes(action.user_decision)).length;
        return <article className="card proposal-card" key={proposal.id}>
        <div className="proposal-head"><div><span className={`proposal-status ${proposal.status}`}>{proposal.status}</span><h2>{proposal.folder_name}</h2><p>Пакет №{proposal.id} · действий {proposal.actions.length} · <strong>{proposal.copy_folder_id.startsWith("virtual:") ? "ОРИГИНАЛ — только одно подтверждённое изменение" : "БЕЗОПАСНАЯ КОПИЯ"}</strong></p></div><div className="proposal-controls">
          ...
          {proposal.status === "waiting_confirmation" && !proposal.copy_folder_id.startsWith("virtual:") && selectedCount > 0 && <button className="apply-safe" disabled={props.busyProposal === proposal.id} onClick={() => props.onConfirmSelected(proposal)}>Подтвердить выбранные ({selectedCount})</button>}
        ...
        <div className="proposal-actions-table" role="region" aria-label="Предпросмотр переименования"><div className="proposal-table-head"><span>Решение</span><span>Было</span><span>Станет</span><span>Уверенность и основание</span></div>
