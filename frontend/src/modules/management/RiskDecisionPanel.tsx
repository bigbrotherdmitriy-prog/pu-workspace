...
export function RiskDecisionPanel({ item, history, historyState, mutationState, mutationMessage, onLoadHistory, onTransition }: Props) {
  ...
  return <section className="management-card" aria-labelledby="governance-title">
    ...
    <div className="management-actions">
      <button type="button" disabled={mutationState === "saving" || !item.evidencePins.length} onClick={() => onTransition(item, confirmStatus)}>Подтвердить</button>
      <button type="button" disabled={mutationState === "saving" || needsReview || !item.evidencePins.length} onClick={() => onTransition(item, closeStatus)}>{item.entityType === "risk" ? "Закрыть риск" : "Зафиксировать исполнение"}</button>
      <button type="button" disabled={mutationState === "saving"} onClick={() => onTransition(item, "dismissed")}>Отклонить</button>
    </div>
