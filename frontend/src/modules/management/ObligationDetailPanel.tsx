...
export function ObligationDetailPanel(props: Props) {
  ...
  return <section className="management-card" aria-labelledby="obligation-title">
    ...
    <div className="management-actions">
      <button type="button" disabled={!canManage || props.mutationState === "saving" || !item.evidencePins.length} onClick={() => props.onTransition(item, "confirmed")}>Подтвердить</button>
      <button type="button" disabled={!canManage || props.mutationState === "saving" || lowConfidence || !item.evidencePins.length} onClick={() => props.onTransition(item, "in_progress")}>В работу</button>
    </div>
