...
type DriveFolder = { id:string; name:string; modifiedTime?:string; registered:boolean };
...
export function App(){
  ...
  async function queueFolder(folder:DriveFolder){try{await api(`/projects/${projectId}/source-folders/${folder.id}/snapshot-queue`,{method:"POST"});setFolders(items=>items.map(item=>item.id===folder.id?{...item,registered:true}:item));await load()}catch(e){setError((e as Error).message)}}
