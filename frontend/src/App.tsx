...
import { AlertTriangle, Bot, CalendarDays, ChevronLeft, FileText, FolderKanban, LayoutDashboard, ListTodo, LogOut, Menu, RefreshCw, Search, Settings, ShieldCheck } from "lucide-react";
...
export function App(){
  ...
  const [projects,setProjects]=useState<Project[]>([]),[projectId,setProjectId]=useState(0),[summary,setSummary]=useState<Summary|null>(null),[documents,setDocuments]=useState<DocumentCard[]>([]),[error,setError]=useState("");
  async function load(){try{const p=await api("/projects/");setProjects(p.projects);const id=projectId||p.projects[0]?.id||0;if(id){setProjectId(id);const d=await api(`/dashboard/project?project_id=${id}`);setSummary(d.summary);setDocuments(d.documents)}}catch(e){setError((e as Error).message)}}
