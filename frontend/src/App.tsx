...
export function App(){
  ...
  return <div className="shell">
    ...
    <main><header><button className="mobile-menu icon" onClick={()=>setMobile(!mobile)}><Menu/></button><div><h1>Рабочий центр</h1><p>Главное по проекту на сегодня</p></div><div className="header-actions"><div className="search"><Search/><input placeholder="Поиск по проекту"/></div><select value={projectId} onChange={e=>setProjectId(Number(e.target.value))}>{projects.map(p=><option value={p.id} key={p.id}>{p.name}</option>)}</select><button className="icon" onClick={load}><RefreshCw/></button></div></header>
      ...
      {showSources&&<section className="card span-3"><div className="card-head"><div><h2>Папки для последовательного разбора</h2><p>Каждая папка получает отдельный виртуальный снимок; файлы не перемещаются.</p></div><button onClick={()=>setShowSources(false)}>Закрыть</button></div><div className="source-list">{folders.map(folder=><article key={folder.id}><FolderKanban/><div><strong>{folder.name}</strong><p>{folder.snapshot_status==="building"?"Создаётся виртуальный снимок…":folder.snapshot_status==="ready"?`Снимок №${folder.snapshot_id} готов · ${(folder.item_count||0).toLocaleString("ru-RU")} объектов`:folder.snapshot_status==="failed"?"Ошибка снимка — можно повторить":folder.modifiedTime?`Изменена ${new Date(folder.modifiedTime).toLocaleDateString("ru-RU")}`:"Google Drive"}</p></div><button disabled={folder.snapshot_status==="building"||folder.snapshot_status==="ready"} onClick={()=>queueFolder(folder)}>{folder.snapshot_status==="building"?"В очереди":folder.snapshot_status==="ready"?"Подключена":folder.snapshot_status==="failed"?"Повторить":"Поставить в очередь"}</button></article>)}</div></section>}
