    ...
    if payload.provider == "google_drive" and payload.connection_id:
        try:
            resolved = DatabaseStorageCredentialPort().resolve(
                db, connection_id=payload.connection_id, project_id=project_id,
                organization_id=project.organization_id, provider="google_drive",
            )
        except StorageCredentialError as exc:
            raise HTTPException(409, "Selected storage credential does not belong to this project") from exc
