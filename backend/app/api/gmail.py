        ...
        try:
            ...
            if existing:
                ...
                if existing.source_sender and not bulk_reason and existing.context_confirmed:
                    discover_contact_from_message(
                        ...
                        mail_connection_id=existing.mail_connection_id,
            ...
            if not bulk_reason and not automated_sender_reason and result.get("context_confirmed"):
                discover_contact_from_message(
                    ...
                    mail_connection_id=stored.mail_connection_id,
...
def send_gmail(draft_id: int, db: Session = Depends(get_db), user: User = Depends(require_user)):
    ...
    try:
        ...
        raise HTTPException(409, "Mailbox origin is unavailable") from exc
    if draft.status == "sent" and action_display_state(db, f"gmail-draft-{draft.id}")["status"] == "applied":
        return {"id": draft.id, "status": "sent", "gmail_message_id": draft.sent_external_id,
                "already_sent": True}
    if draft.status not in {"approved", "queued", "sending", "failed"} or draft.approved_revision != draft.revision:
