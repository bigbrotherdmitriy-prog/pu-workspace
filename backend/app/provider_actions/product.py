...
from app.mail_content import is_rich_mail_body, mail_html_to_text
...
def _project_token_material(db, *, project: Project, actor_id: int, action_kind: str,
    ...
    return _Material(
        ...
        authority_epoch=version, capability_version=1,
        credential_generation=int(token.credential_generation or 1),
...
def _addresses(value: str | None) -> tuple[str, ...]:
    ...
    try:
        ...
        pass
    return tuple(address.strip().casefold() for _, address in getaddresses([value]) if address.strip())
...
def _gmail_payload(draft: ResponseDraft, recipient: str, thread_id: str | None) -> dict:
    ...
    return {
        ...
        "recipient": recipient, "cc": _addresses(draft.recipient_cc),
        "bcc": _addresses(draft.recipient_bcc), "subject": subject, "body": draft.body,
...
def _gmail_material(db, draft: ResponseDraft, actor_id: int) -> _Material:
    if (draft.approved_revision != draft.revision or draft.approved_by_user_id is None
            or draft.attachments_json not in (None, "", "[]")):
    ...
    if source is not None and source.mail_connection_id:
        ...
        pins = (f"source:{runtime.source_reference_id}@{runtime.source_version_id}",
                ...
                f"approval:{draft.approved_by_user_id}@{draft.revision}")
        return _Material(
            organization_id=project.organization_id, project_id=project.id,
...
def _material_for(db, *, action_kind: str, target_id: int, actor_id: int) -> _Material:
    if action_kind == "gmail.message.send":
        ...
        if draft is None or draft.status not in {"approved", "queued", "sending", "sent", "unknown", "failed"}:
            raise ProviderActionError("payload_stale")
        ...
        with self.sessions.begin() as db:
            ...
            if request.action_kind == "gmail.message.send":
                ...
                if draft.sent_external_id:
                    found = service.users().messages().get(userId="me", id=draft.sent_external_id).execute()
                    if found.get("id") == draft.sent_external_id:
                        return self._receipt(request, "APPLIED", draft.sent_external_id)
                    raise ProviderPreconditionFailed()
                source = db.get(Message, draft.message_id) if draft.message_id else None
                recipient = ", ".join(_addresses(draft.recipient_to) or
                                      _addresses(parseaddr(source.source_sender)[1] if source else ""))
                ...
                if canonical["cc"]:
                    message["Cc"] = ", ".join(canonical["cc"])
                if canonical["bcc"]:
                    message["Bcc"] = ", ".join(canonical["bcc"])
                message["Message-ID"] = self._gmail_message_id(request)
                if is_rich_mail_body(draft.body):
                    message.set_content(mail_html_to_text(draft.body))
                    message.add_alternative(draft.body, subtype="html")
                else:
                ...
                verified = service.users().messages().get(userId="me", id=external_id).execute()
                if str(verified.get("id")) != external_id:
                    raise RuntimeError("provider_readback_unavailable")
                ...
                verified = service.tasks().get(tasklist=container, task=external_id).execute()
                if str(verified.get("id")) != external_id or verified.get("deleted") is True:
                    raise RuntimeError("provider_readback_unavailable")
                ...
                verified = service.events().get(calendarId="primary", eventId=external_id).execute()
                if str(verified.get("id")) != external_id or verified.get("status") == "cancelled":
                    raise RuntimeError("provider_readback_unavailable")
    ...
    def lookup(self, request: ProviderRequest) -> ProviderReceipt | None:
        with self.sessions.begin() as db:
            ...
            if request.action_kind == "gmail.message.send":
                ...
                if external_id:
                    try:
                        found = service.users().messages().get(userId="me", id=external_id).execute()
                        if found.get("id") != external_id:
                            return None
...
def queue_reconciliation(db, *, action_id: str, revision: int, actor: User) -> dict:
    ...
    if row is None or row.provider != "google_workspace" or row.state not in {"UNKNOWN", "APPLIED"}:
...
def _project_outcome(sessions, payload: dict, result: dict) -> None:
    ...
    with sessions.begin() as db:
        ...
        if row.action_kind == "gmail.message.send":
            draft = db.get(ResponseDraft, target_id)
            if draft:
                draft.status = {"APPLIED": "sent", "NOT_APPLIED": "failed",
                                "UNKNOWN": "unknown"}.get(outcome, draft.status)
                if outcome != "APPLIED":
                    draft.last_send_error_code = "provider_outcome_unknown" if outcome == "UNKNOWN" else "provider_not_applied"
            return
        task = db.get(Task, target_id)
        if task:
            states = task_effect_states(db, target_id)
            task.external_action_status = task_overall_state(states)


def action_display_state(db, action_id: str) -> dict:
    """The UI trusts ledger observations, never just a stored external ID."""
    row = db.scalar(select(ProviderAction).where(
        ProviderAction.action_id == action_id,
    ).order_by(ProviderAction.revision.desc()).limit(1))
    if row is None:
        return {"status": "not_requested", "external_id": None}
...
def task_effect_states(db, task_id: int) -> dict:
    return {
        "task": action_display_state(db, f"google-task-{task_id}"),
        "calendar": action_display_state(db, f"google-calendar-{task_id}"),
    }


def task_overall_state(states: dict) -> str:
    active = [item["status"] for item in states.values() if item["status"] != "not_requested"]
    if not active:
        return "proposed"
    if "unknown" in active:
        return "unknown"
    if "executing" in active:
        return "executing"
    if "pending" in active:
        return "queued"
    if "failed" in active:
        return "failed"
    return "executed" if all(status == "applied" for status in active) else "unknown"
