...
HEADER_ALIASES = {
    ...
    "direction": ("направление", "приход расход", "тип платежа", "операция"),
...
def parse_structured_rows(content: str, kind: str, limit: int = 500) -> dict:
    ...
    rows = []
    for source_row, cells in enumerate(matrix[header_index + 1:header_index + 1 + limit], start=header_index + 2):
    ...
    return {
        ...
        "rows": rows,
        "issues": issues,
        "truncated": len(matrix) > header_index + 1 + limit,
    }
