...
def test_google_list_page_size_is_strictly_bounded():
    files = Files(list_outcomes=[{"files": [], "nextPageToken": None}])
    DriveClient(Service(files), sleep=lambda _: None).list_children("root")
    assert files.list_kwargs[0]["pageSize"] == 1000
...
def test_shortcut_metadata_migration_is_sequential_current_head():
    ...
    assert scripts.get_heads() == ["201286e2acd0"]
    assert scripts.get_revision("201286e2acd0").down_revision == "21d8f9354c18"
